...
const ConvertCandidateStatus_1 = require("./convert/ConvertCandidateStatus");
class MinterHttpApi {
    httpOptions;
    nodeUrl;
    ...
    getCandidatesGrpc(includeStakes, notShowStakes, candidateStatus, height, timeout) {
    ...
    urlCandidate(request) {
        const params = [];
        if (request.getHeight())
            params.push({ height: request.getHeight().toString() });
        if (request.getNotShowStakes() === true)
            params.push({ not_show_stakes: "true" });
        else if (request.getNotShowStakes() === false)
            params.push({ not_show_stakes: "false" });
        return this.url(this.nodeUrl + "candidate/" + request.getPublicKey(), params);
    }
    getCandidateJsonByRequest(request, timeout = null) {
        return this.httpGet(this.urlCandidate(request), timeout);
    }
    getCandidateGrpcByRequest(request, timeout = null) {
        return new Promise((resolve, reject) => {
    ...
    getCandidateGrpc(publicKey, notShowStakes = null, height = null, timeout = null) {
    ...
    getCandidatesJsonByRequest(request, timeout = null) {
        return this.httpGet(this.urlCandidates(request), timeout);
    }
    getCandidatesGrpcByRequest(request, timeout = null) {
        return new Promise((resolve, reject) => {
    ...
    urlCandidates(request) {
        ...
        if (request.getNotShowStakes() === true)
            ...
            params.push({ not_show_stakes: "false" });
        if (request.getIncludeStakes() === true)
            params.push({ include_stakes: "all" });
        else if (request.getIncludeStakes() === false)
            params.push({ include_stakes: "false" });
        const status = new ConvertCandidateStatus_1.default().get(request.getStatus());
        if (status)
